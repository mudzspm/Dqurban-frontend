...
import { SERVER_URL } from "../../../constant/ServerUrl.js";
// @ts-ignore
...
const SadaqahInvoiceInstallment = () => {
    ...
    <br></br><br></br>
      ...
      const previousDueDate = index === 0 
        ? new Date(orderDetails?.created_at) 
        : new Date(orderDetails?.installments[index - 1].due_date);
      
      // Increment the previous due date by one day
      const nextDueDate = new Date(previousDueDate);
      nextDueDate.setDate(nextDueDate.getDate() + 1);
    
    
      // Get the month name in Malay
      const monthName = nextDueDate.toLocaleString('ms-MY', { month: 'long' }); // e.g., "Jun", "Julai"
    
      // Determine the ordinal in Malay
      const ordinal = index === 0 ? 'pertama' 
                     : index === 1 ? 'kedua' 
                     : index === 2 ? 'ketiga' 
                     : `ke-${index + 1}`; // e.g., "pertama", "kedua", "ketiga", "ke-4", etc.
    
                     const textColor = installment.status.toLowerCase() === "paid" ? 'green' : 'inherit'; // green text for "Terima Bayaran"
    
    
      return (
        <tr key={index}>
          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
            {index === 0 ? formatDate(orderDetails?.created_at) : formatDate(previousDueDate)}
          </td>
          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
            {formatDate(installment?.due_date)}
          </td>
          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
            {`Ansuran - ${orderDetails?.installments?.length} bulan`}
          </td>
          <td style={{ padding: '8px', border: '1px solid #ddd' }}>
          {`${monthName} - ${ordinal}`}
          </td>
          <td style={{ padding: '8px', border: '1px solid #ddd', color: textColor  }}>
            {installment?.status?.toLowerCase() ==="pending" ? "Sedang diproses" : "Terima Bayaran"}
          </td>
        </tr>
      );
    })}
    ...
    {/* {orderDetails?.principal_portions?.map((item) => {
      ...
      return ( */}
        <tr >
          ...
          <td style={{ padding: '8px', border: '1px solid #ddd' }}>{orderDetails?.package?.package?.pkg_type.toLowerCase() === "waqf" ? "sadaqah" : ""}</td>
    ...
    {/* {orderDetails.OrderHistoryOrganization.map((item) => {
      ...
      return (
        <tr key={`organization_${currentIndex}`}>
          ...
          <p style={{ marginRight: '10px', marginTop: "15px" }}>Jumlah keseluruhan (RM) :</p>
          <p style={{ color: '#1a202c',marginTop: "15px",marginLeft: '60px' }}>{formatPrice(parseFloat(orderDetails.price))}</p>
        </div>
        {
      orderDetails.installment_option && (
        <div style={{ textAlign: 'right', display: 'flex', justifyContent: 'flex-end' }}>
          <p style={{ marginRight: '10px', fontWeight: 'bold',marginTop: "12px" }}>Jumlah sudah dibayar (RM) :</p>
          <p style={{ color: '#1a202c',marginTop: "12px",marginLeft: '60px'}}> {formatPrice(orderDetails?.installments
            ?.filter(installment => installment.status.toLowerCase() === "paid")
            ?.reduce((total, installment) => total + parseFloat(installment.total_payable), 0))}</p>
        </div>
         )
    }
        <div style={{ textAlign: 'right', display: 'flex', justifyContent: 'flex-end' }}>
          <p style={{ marginRight: '10px', fontWeight: 'bold',marginTop: "12px" }}>Jumlah perlu dibayar (RM) :</p>
          <p style={{ color: '#1a202c',marginTop: "12px", marginLeft: '60px'}}>{formatPrice(
        parseFloat(orderDetails.price) - 
        orderDetails?.installments
          ?.filter(installment => installment.status.toLowerCase() === "paid")
          ?.reduce((total, installment) => total + parseFloat(installment.total_payable), 0)
      )}</p>
        </div> */}
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      ...
      <p style={{ color: '#1a202c', marginTop: "15px", textAlign: 'right',width: '26%' }}>{formatPrice(parseFloat(orderDetails?.price))}</p>
    </div>
    {
      orderDetails?.installment_option && (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <p style={{ marginRight: '10px', fontWeight: 'bold', marginTop: "12px",width: '73%',textAlign: 'right' }}>Jumlah sudah dibayar (RM) :</p>
          <p style={{ color: '#1a202c', marginTop: "12px", textAlign: 'right', minWidth: '200px',width: '26%' }}>
            {formatPrice(orderDetails?.installments
              ?.filter(installment => installment?.status?.toLowerCase() === "paid")
              ?.reduce((total, installment) => total + parseFloat(installment?.total_payable), 0))}
          </p>
        </div>
...
export default SadaqahInvoiceInstallment